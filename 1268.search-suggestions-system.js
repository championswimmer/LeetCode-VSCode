...
var suggestedProducts = function(products, searchWord) {
  products.sort();
  const result = new Array(searchWord.length).fill(0).map(() => []);
  for (let i = 0; i < searchWord.length; i++) {
    const currPrefix = searchWord.slice(0, i + 1);
    if (i === 0) {
      result[i] = products.filter((product) => product.startsWith(currPrefix));
    } else {
      result[i] = result[i - 1].filter((product) => product.startsWith(currPrefix));
    }
  }
